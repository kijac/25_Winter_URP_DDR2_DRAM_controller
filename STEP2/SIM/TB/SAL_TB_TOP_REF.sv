...
module SAL_TB_TOP_REF;
    ...
    initial begin
        ...
        // Force refresh timer to a small value to trigger refresh quickly
        // Wait until normal initialization is done (which is included in init())
        $display("Forcing refresh timer to trigger refresh...");
        force u_dram_ctrl.timer_ref = 32'd50;
        @(posedge clk);
        release u_dram_ctrl.timer_ref;
        ...
        repeat (2000) @(posedge clk); // extended wait to see more refreshes if needed
        $display("Succeed!");
        $finish;
    end

    // Monitor for Refresh command
    always @(posedge clk) begin
        if (!dfi_ctrl_if.cs_n[0] && !dfi_ctrl_if.ras_n && !dfi_ctrl_if.cas_n && dfi_ctrl_if.we_n) begin
            $display("Time %t: Refresh Command Detected!", $time);
        end
    end

endmodule // SAL_TB_TOP_REF
